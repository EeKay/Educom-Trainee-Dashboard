...
function resolveCollisions(idealPositions, entries) {
  ...
  const gapBefore = (i) =>
    BASE_GAP +
    (entries[i].isYou || entries[i - 1].isYou ? YOU_GAP_BONUS : 0);

  for (let i = 1; i < n; i++) {
    if (positions[i] - positions[i - 1] < gapBefore(i)) {
      positions[i] = positions[i - 1] + gapBefore(i);
    }
  }

  for (let i = n - 2; i >= 0; i--) {
    if (positions[i + 1] - positions[i] < gapBefore(i + 1)) {
      positions[i] = positions[i + 1] - gapBefore(i + 1);
    }
  }
...
export default function LeaderboardAdmin({
  ...
  const positions = resolveCollisions(idealPositions, ranked);
          ...
          >
            ...
            {ranked.map((user, i) => (
              ...
              <div className="leaderboard-avatar-wrapper">
                <div
                  className="leaderboard-avatar-frame"
